...
var thresholdTracker = thresholdtracker.NewThresholdTracker()
...
func matchIP(ruleIP, packetIP string) bool {
	...
	if ruleIP == "$HOME_NET" {
		for _, cidrStr := range sethomenet.HomeNet {
...
func MatchPacket(pkt Packet, rule loadrulefiles.LoadedJsonRules) bool {
...
// Modified matching function to integrate IP blocking
func Matching(rules []loadrulefiles.LoadedJsonRules, protocol *string, sip *string, sport *int, dip *string, dport *int, payload *[]byte, flow *string, flags *string, pktFlowbits *[]string) {
    // Initialize IP blocker if not done already
    if IpBlocker == nil {
    	InitIPBlocker()
    }
        ...
        if MatchPacket(pkt, rule) {
            ...
            if rule.Threshold != nil {
                ...
                if err != nil {
                    ...
                    log.Println("Second threshold fallback to 60")
                }
