...
// 	var rules []loadrulefiles.LoadedJsonRules
// 	if doMatch {
// 		rules, err = loadrulefiles.LoadRules(rulePath)
// 		if err != nil {
...
// PacketChannel is a buffered channel used to stream captured packets.
var PacketChannel = make(chan CapturedPacket, 100)
...
// CaptureAndLogAllFields listens for packets on the specified interface,
// extracts various fields, performs flow tracking, and (if matching is disabled)
// logs the packet. It has been augmented (without disturbing the existing flow)
// to also send each captured packet to PacketChannel for WebSocket streaming.
func CaptureAndLogAllFields(iface string, doMatch bool, rulePath string) error {
	handle, err := pcap.OpenLive(iface, 1600, true, pcap.BlockForever)
	if err != nil {
		return fmt.Errorf("failed to open interface: %v", err)
	...
	for packet := range packetSource.Packets() {
		...
		// --- New Addition ---
		// Send the captured packet to PacketChannel for WebSocket streaming.
		PacketChannel <- capturedPkt
	}

	return nil
}

// CapturePacketsStream starts packet capture in a goroutine.
func CapturePacketsStream(iface string, doMatch bool, rulePath string) {
	go func() {
		err := CaptureAndLogAllFields(iface, doMatch, rulePath)
		if err != nil {
			log.Println("Error capturing packets:", err)
		}
	}()
}

// SendPacketsToWebSocket sends captured packets from PacketChannel to a WebSocket connection.
func SendPacketsToWebSocket(conn *websocket.Conn) {
	for pkt := range PacketChannel {
		err := conn.WriteJSON(pkt)
		if err != nil {
